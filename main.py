...
try:
    ...
    print("Fyers API libraries not found. Please ensure 'fyers-api' is installed.")
...
FYERS_SECRET_KEY = os.environ.get('FYERS_SECRET_KEY') # Usually not directly used in bot, but good to have

# Ensure essential environment variables are set
if not FYERS_APP_ID or not FYERS_ACCESS_TOKEN or not FYERS_SECRET_KEY:
    logging.error("Missing one or more Fyers environment variables (FYERS_APP_ID, FYERS_ACCESS_TOKEN, FYERS_SECRET_KEY). Please set them.")
    exit(1)
else:
...
completed_candle_history = {} # Stores completed 1-minute candles for SMA calculation
...
def onmessage(message):
    ...
    if isinstance(message, dict):
            ...
            for candle_data in message.get('v', []):
                ...
                if symbol:
                    ...
                    if timestamp_ist_epoch:
                        timestamp_utc = datetime.fromtimestamp(timestamp_ist_epoch, tz=pytz.timezone('Asia/Kolkata')).astimezone(pytz.utc)
                        ...
                        live_candles[symbol] = {
                            ...
                            'volume': candle_data.get('vol'),
                            'cumulative_day_volume': candle_data.get('short_mkt_qty') # For cumulative day volume
                        }
        ...
        logging.warning(f"Unexpected message type from WebSocket: {type(message)} - {message}")
...
def onopen():
    ...
    logging.info(f"WebSocket connection established. Subscribing to instruments...")
    fyers_ws_client.subscribe(symbols=symbols_to_subscribe, data_type=data_type)
...
def authenticate_fyers(app_id, secret_key, access_token):
    ...
    try:
        ...
        logging.error(f"Error during Fyers authentication or profile fetch: {e}")
        return None
...
def initialize_fyers_client(access_token):
    ...
    try:
        ...
        logging.info(f"Using WebSocket token format: {FYERS_APP_ID}:<your_token_starts_here...> (masked for security)")
...
def check_sma_crossover(symbol):
    ...
    if len(history) < SMA_SLOW_PERIOD + 1: # Need previous candle for previous SMA calculation
        return None

    prev_history = history[:-1] # History excluding the very last (current) candle
    ...
    if fast_sma > slow_sma and prev_fast_sma <= prev_slow_sma:
        logging.info(f"STRATEGY SIGNAL for {symbol}: BUY (Fast SMA crossed above Slow SMA)")
        return "BUY"
    elif fast_sma < slow_sma and prev_fast_sma >= prev_slow_sma:
        logging.info(f"STRATEGY SIGNAL for {symbol}: SELL (Fast SMA crossed below Slow SMA)")
        return "SELL"
    return None
...
def execute_trade(symbol, signal, price):
    ...
    if not fyers_rest_client:
        logging.error("Fyers REST client not initialized. Cannot execute trade.")
        return
    ...
    order_params = {
        ...
        "type": 2, # 1: Limit Order, 2: Market Order, 3: Stop Order, 4: Stop Limit Order
    ...
    try:
        logging.info(f"Attempting to place {signal} order for {symbol} at price {price}...")
        response = fyers_rest_client.place_order(data=order_params)
        if response and response.get('s') == 'ok':
            order_id = response.get('id')
            logging.info(f"Successfully placed {signal} order for {symbol}. Order ID: {order_id}")
            logging.info(f"Order response: {response}")
        else:
            logging.error(f"Failed to place {signal} order for {symbol}: {response}")
    except Exception as e:
...
async def main():
    ...
    fyers_rest_client = authenticate_fyers(FYERS_APP_ID, FYERS_SECRET_KEY, FYERS_ACCESS_TOKEN)
    if not fyers_rest_client:
    ...
    for symbol in symbols_to_subscribe:
        ...
        live_candles[symbol] = {} # Initialize empty live candle for each symbol
        ...
        if market_open_time_utc <= current_time_utc < market_close_time_utc:
            ...
            if last_minute_checked is None or current_minute_start > last_minute_checked:
                for symbol in list(live_candles.keys()):
                    candle = live_candles.get(symbol) # Use .get to avoid KeyError if symbol not yet updated
            ...
            logging.info(f"Market not yet open. Current UTC time: {current_time_utc.strftime('%H:%M:%S')}. Waiting for market open (UTC {market_open_time_utc.strftime('%H:%M')}).")
